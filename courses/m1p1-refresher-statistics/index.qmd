...
::: proof
**Complement.** $\Omega = A \sqcup A^c$, hence $1 = \mathbb{P}(A) + \mathbb{P}(A^c)$.

**Monotonicity.** $B = A \sqcup (B \setminus A)$ and $\mathbb{P}(B\setminus A) \geq 0$.

**Union bound.** $A \cup B = A \sqcup (B \setminus A)$ and $B \setminus A \subseteq B$.

**Increasing continuity.** Set $C_1 = A_1$, $C_n = A_n \setminus A_{n-1}$; the $C_n$ are disjoint, with union $A$, and $\sum_{k \leq n} \mathbb{P}(C_k) = \mathbb{P}(A_n)$; conclude by σ-additivity. Decreasing continuity follows by taking complements. [∎]{.qed}
:::
...
::: proof
**Monotonicity.** $\{X \leq s\} \subseteq \{X \leq t\}$.

**Difference.** $\{X \leq b\} = \{X \leq a\} \sqcup \{a \lt X \leq b\}$.

**Right-continuity.** $\{X\leq t+1/m\}\downarrow\{X\leq t\}$.

**Limits.** $\{X \leq n\} \uparrow \Omega$ and $\{X \leq -n\} \downarrow \varnothing$.

**Point mass.** $\{a - \tfrac1m \lt X \leq a\} \downarrow \{X = a\}$.

The converse follows from the measure extension theorem, admitted. [∎]{.qed}
:::
...
::: proof
**$L^p \Rightarrow$ probability.** Markov on $|X_n-X|^p$.

**a.s. $\Rightarrow$ probability.** $A_n(\varepsilon) = \bigcup_{k \geq n}\{|X_k - X| > \varepsilon\}$ decreases towards the event "deviations $> \varepsilon$ occur infinitely often", of probability zero; $\mathbb{P}(|X_n - X| > \varepsilon) \leq \mathbb{P}(A_n) \to 0$.
