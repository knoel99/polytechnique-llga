...
![Bayes tree for the medical test (joints TP/FP)](schema-bayes-tree.svg){fig-alt="Bayes medical test probability tree" width="100%"}
...
\end{aligned}
$$
Check: $\mathbb{E}[(X-\tfrac53)^2] = (\tfrac{1-5/3})^2\cdot\tfrac12 + (\tfrac{2-5/3})^2\cdot\tfrac13 + (\tfrac{3-5/3})^2\cdot\tfrac16 = \tfrac59$.
:::
...
::: ex
[Example 2.10b — Markov with numbers]{.tag} Nonnegative $X$ with $\mathbb{E}[X]=2$. Bound $\mathbb{P}(X\geq 10)$: $\mathbb{P}(X\geq 10)\leq 2/10=0.2$. If also $\mathrm{Var}(X)=1$, Chebyshev gives $\mathbb{P}(|X-2|\geq 3)\leq 1/9\approx 0.111$ — often tighter when a variance is known.
:::
...
![From a flat density to a Gaussian histogram of means](schema-clt.svg){fig-alt="CLT schematic" width="100%"}

::: ex
[Example 4.4b — Fair die, written CLT table]{.tag} Fair die: $\mu=3.5$, $\sigma^2=35/12\approx 2.9167$. For $n$ iid rolls, theory gives $\mathrm{sd}(\bar X_n)=\sigma/\sqrt{n}$. One Monte Carlo (2000 repeats, seed 0) vs theory:
...
::: ex
[Example 5.0b — Gaussian MLE with a 4-point sample]{.tag} Observations $x=(2.0,\,2.5,\,1.5,\,2.0)$. For $\mathcal{N}(\mu,\sigma^2)$ with both unknown:
$$
...
$$
(The unbiased $s^2$ would divide by $n-1=3$: $s^2=0.5/3\approx 0.1667$.) Log-likelihood at this point: $\ell=-\tfrac{n}{2}\log(2\pi\hat\sigma^2)-\tfrac{n}{2}$.
:::
...
![Pivot $Z$ and the resulting CI for $\mu$](schema-ci-pivot.svg){fig-alt="CI via normal pivot" width="100%"}
...
::: ex
[Example 6.1b — $z$-interval, step table]{.tag} Known $\sigma=2$, $n=25$, $\bar x=10.4$, level $95\%$ so $z_{0.025}=1.96$.
...
::: ex
[Example 6.3b — $t$-interval vs $z$, same numbers]{.tag} Now $\sigma$ unknown, $s=2.0$, $n=25$, $\bar x=10.4$, $95\%$. df $=24$, $t_{24,0.025}\approx 2.064$.
